...
|------|----------------------------------------|-----------------------------------|------------------|
| 执行模型 | 无限步数 + 自适应停止 | `streamText + stopWhen: adaptive cap=999` | 自适应停止 + 硬上限 `999` |
...
这会消耗额外 step（即便 cap=999 仍浪费预算），且用户体验冗长。
...
| 优先级 | 升级方向 | 投入 | 收益 |
|--------|---------|------|------|
| **P0** | 固定步数 → 自适应停止 + cap=999 | 1行代码 | 解锁多步编排能力 |
